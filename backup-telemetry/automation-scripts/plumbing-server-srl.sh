...
config_server_frontend_and_rdma() {
  ...
  # Backend RDMA ports
  # Routed-style backend:
  #   server1/server2 use VLAN 101
  #   server3/server4 use VLAN 201
  # Each server-facing rail port gets its own IPv4 + IPv6 subnet.
  # RXE is bound to the VLAN subinterface, not the raw eth11/eth12 parent.
  local rdma_vlan
  ...
  case "$id" in
    1)
      ...
      eth11_v6="fd00:101:11::1/64"; eth11_gw6="fd00:101:11::ff"
      eth12_v6="fd00:101:12::1/64"; eth12_gw6="fd00:101:12::ff"
      ;;
    2)
      rdma_vlan=101
      eth11_v4="192.168.111.2/24"; eth11_gw4="192.168.111.254"
      eth12_v4="192.168.112.2/24"; eth12_gw4="192.168.112.254"
      eth11_v6="fd00:101:21::2/64"; eth11_gw6="fd00:101:21::ff"
      eth12_v6="fd00:101:22::2/64"; eth12_gw6="fd00:101:22::ff"
      ;;
    3)
      rdma_vlan=201
      eth11_v4="192.168.201.3/24"; eth11_gw4="192.168.201.254"
      eth12_v4="192.168.202.3/24"; eth12_gw4="192.168.202.254"
      eth11_v6="fd00:201:31::3/64"; eth11_gw6="fd00:201:31::ff"
      eth12_v6="fd00:201:32::3/64"; eth12_gw6="fd00:201:32::ff"
      ;;
    4)
      rdma_vlan=201
      eth11_v4="192.168.211.4/24"; eth11_gw4="192.168.211.254"
      eth12_v4="192.168.212.4/24"; eth12_gw4="192.168.212.254"
      eth11_v6="fd00:201:41::4/64"; eth11_gw6="fd00:201:41::ff"
      eth12_v6="fd00:201:42::4/64"; eth12_gw6="fd00:201:42::ff"
      ;;
    *)
  ...
  eth11_vif="eth11.${rdma_vlan}"
  eth12_vif="eth12.${rdma_vlan}"

  nsenter -t "$pid" -n ip link set eth11 up
  nsenter -t "$pid" -n ip link set eth12 up

  nsenter -t "$pid" -n ip link add link eth11 name "$eth11_vif" type vlan id "$rdma_vlan" 2>/dev/null || true
  nsenter -t "$pid" -n ip link add link eth12 name "$eth12_vif" type vlan id "$rdma_vlan" 2>/dev/null || true
  ...
  nsenter -t "$pid" -n ip -6 addr add "$eth11_v6" dev "$eth11_vif" 2>/dev/null || true
  nsenter -t "$pid" -n ip -6 addr add "$eth12_v6" dev "$eth12_vif" 2>/dev/null || true

  nsenter -t "$pid" -n ip link set "$eth11_vif" up
  nsenter -t "$pid" -n ip link set "$eth12_vif" up

  # Optional broad routes. These assume rail1/rail2 L3 gateways are configured.
  # eth11/rail1 is primary; eth12/rail2 is added with higher metric.
  ...
  nsenter -t "$pid" -n ip -6 route replace fd00::/8 via "$eth11_gw6" dev "$eth11_vif" metric 101 || true
  nsenter -t "$pid" -n ip -6 route replace fd00::/8 via "$eth12_gw6" dev "$eth12_vif" metric 201 || true

  # RXE must be created on VLAN interfaces.
...
echo "  docker exec -it server2 ib_write_bw -d rxe_server2_11 192.168.101.1"
