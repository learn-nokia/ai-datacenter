...
delete_if_in_container() {
  ...
  nsenter -t "$pid" -n ip link del "$ifname" 2>/dev/null || true
}
...
cleanup_links() {
  echo "[cleanup] deleting old veths from host namespace..."
  ...
  echo "[cleanup] deleting old server-side interfaces..."

  for s in server1 server2 server3 server4; do
    delete_if_in_container "$s" eth1
    delete_if_in_container "$s" eth2
    delete_if_in_container "$s" eth11
    delete_if_in_container "$s" eth12
    delete_if_in_container "$s" bond0.1001
    delete_if_in_container "$s" bond0.201
    delete_if_in_container "$s" bond0.202
    delete_if_in_container "$s" bond0
  done
...
plug_link() {
  ...
  # Defensive cleanup before creating this specific link
  delete_if_in_host "$host_tmp"
  delete_if_in_host "$srl_tmp"
  delete_if_in_container "$srv" "$srv_if"
...
config_server_frontend_and_rdma() {
  ...
  # Remove stale old broad IPv6 routes
  nsenter -t "$pid" -n ip -6 route del fd00::/8 2>/dev/null || true
  nsenter -t "$pid" -n ip -6 route del fd00::/8 2>/dev/null || true
  nsenter -t "$pid" -n ip -6 route del fd00::/8 2>/dev/null || true
  nsenter -t "$pid" -n ip -6 route del fd00::/8 2>/dev/null || true

  # Explicit IPv6 static routes
  case "$id" in
    1)
