...
### Making sping and connect to non-root

```
# 1. Create the .kube configuration folder inside their home directory
sudo mkdir -p /home/nokiauser/.kube

# 2. Copy the active cluster configuration file over
sudo cp -i /root/.kube/config /home/nokiauser/.kube/config

# 3. Change file ownership entirely to nokiauser
sudo chown -R nokiauser:nokiauser /home/nokiauser/.kube

# 4. Strict Permissions: Lock the files down so other regular users can't read their keys
sudo chmod 700 /home/nokiauser/.kube
sudo chmod 600 /home/nokiauser/.kube/config
```
